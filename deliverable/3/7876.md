...
In addition to the changes proposed, a similar approach to the second option can be taken for the error `nan`. A special marker or symbol can be displayed on the data point to explicitly indicate its error type. In that case, the user could add special markers for both a `nan` and `inf` error for a better representation. Moreover, these markers could be added to the `Legend` to indicate their meaning.
